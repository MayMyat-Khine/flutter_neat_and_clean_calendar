...
import 'package:hifive/MOM/Models/employeeListModel.dart';
import 'package:hifive/MOM/View/Meeting/sharedMOMDetailWidget.dart';
import 'package:hifive/Utils/global_data.dart';
...
class Calendar extends StatefulWidget {
  ...
  final double? eventTileHeight;
  final List<int>? hasAccessListfinal;
  final List<String>? hasViewMomList;
  final String empId;
        ...
        child: _selectedEvents != null && _selectedEvents!.isNotEmpty
            ? ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  return Container(
                    height: widget.eventTileHeight ??
                        MediaQuery.of(context).size.height * 0.075, //0.080
                    child: InkWell(
                      onTap: () {
                        if (widget.onEventSelected != null) {
                          widget.onEventSelected!(event);
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          ...
                          (event.color == Colors.green &&
                                  widget.hasViewMomList!.contains(widget.empId +
                                      _selectedEvents![index].id.toString()))
                              // widget.hasViewMomList!
                              //     .contains(_selectedEvents![index].id))

                              // hasAccessList
                              //   .contains(_selectedEvents![index].id)
                              //   widget.hasViewMomList!.contains({
                              //   GlobalData
                              //       .profile?.employeeInfo?.employeeId
                              // }.toString() +
                              // _selectedEvents![index].id.toString())
                              ? IconButton(
                                  onPressed: () {
                                    print(GlobalData
                                        .profile?.employeeInfo?.employeeId);
                                    print("event ID >>>>>>>>>>>>>>>>>.");
                                    print(_selectedEvents![index].id);

                                    Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                            builder: (context) =>
                                                SharedMOMDetailedWidget(
                                                  title: _selectedEvents![index]
                                                      .description,
                                                  momid: _selectedEvents![index]
                                                      .id,
                                                  allemployees:
                                                      EmployeeListModel
                                                          .listtemp,
